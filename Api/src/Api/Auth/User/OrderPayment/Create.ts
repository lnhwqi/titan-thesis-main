  ...
  try {
    const createdRows = await db.transaction().execute(async (trx) => {
      ...
      if (lineItems.length === 0) {
        const emptyRows: OrderPaymentRow.OrderPaymentRow[] = []
        return emptyRows
      }
      ...
      const created: OrderPaymentRow.OrderPaymentRow[] = []
      ...
      for (const panel of panels) {
        ...
        created.push(OrderPaymentRow.orderPaymentRowDecoder.verify(inserted))
      }
    ...
    return ok({ orderPayments: createdRows.map(toOrderPayment) })
  } catch (e) {
