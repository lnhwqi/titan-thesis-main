...
import { numberStringDecoder } from "../../Data/Number"

const key: unique symbol = Symbol()
export type Rating = Opaque<number, typeof key>
export type ErrorRating = "INVALID_PRODUCT_RATING"
...
function _validate(s: number): Result<ErrorRating, number> {
  if (typeof s !== "number" || Number.isInteger(s) === false || isNaN(s)) {
    return err("INVALID_PRODUCT_RATING")
  }

  if (s < 0 || s > 5) return err("INVALID_PRODUCT_RATING")

  return ok(s)
}

export const ratingDecoder: JD.Decoder<Rating> = JD.number.transform((s) => {
  return throwIfNull(createRating(s), `Invalid product rating: ${s}`)
})

export const ratingStringDecoder: JD.Decoder<Rating> =
  numberStringDecoder.transform((s) => {
    return throwIfNull(createRating(s), `Invalid product rating string: ${s}`)
  })
