        ...
        {state.report.sellerReports.map((report) => {
          ...
          return (
            <article key={id} className={styles.card}>
              ...
              <div>
                User Images: {report.userUrlImgs.map((i) => i.unwrap()).join(", ") || "-"}
              </div>
...
function sellerHint(status: ReportStatus, hasEvidenceInput: boolean): string {
  ...
  if (status === "CASHBACK_COMPLETED" || status === "RESOLVED" || status === "REJECTED") {
...
const styles = {
  ...
  sectionTitle: css({ ...font.bold14, marginTop: theme.s1, color: color.neutral800 }),
  ...
  secondaryButton: css({ border: `1px solid ${color.secondary300}`, background: color.neutral0, color: color.secondary500, borderRadius: theme.s2, padding: `${theme.s2} ${theme.s3}`, cursor: "pointer", width: "fit-content", "&:disabled": { opacity: 0.6, cursor: "not-allowed" } }),
